...
<div class="table-card">
    <div class="table-card-header">
        ...
        @can('create service-returns')
        <a href="{{ route('service-returns.create') }}" class="btn btn-sm btn-primary">
            ...
            <div class="row g-2 align-items-end">
                <div class="col-md-4">
                    <div class="input-group input-group-sm">
                        ...
                        <input type="text" class="form-control" name="search" value="{{ request('search') }}" placeholder="Return #, service #, customer...">
                    </div>
                </div>
                <div class="col-md-2">
                ...
                <div class="col-md-3 d-flex gap-2">
                    ...
                    <a href="{{ route('service-returns.index') }}" class="btn btn-sm btn-outline-secondary">Clear</a>
                    @endif
                </div>
            </div>
        </form>
        ...
        <table class="table table-hover mb-0">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <a href="{{ route('services.show', $return->service_id) }}" class="text-primary fw-semibold" style="font-size:13px;">
                            {{ $return->service->service_number }}
                        </a>
                        <div style="font-size:11px;color:#9ca3af;">{{ $return->service->product_name }}</div>
                    </td>
                    <td style="font-size:13px;">{{ $return->service->customer_name }}</td>
                    <td style="font-size:13px;">{{ $return->return_date->format('d M Y') }}</td>
                    <td class="text-end fw-bold">৳{{ number_format($return->refund_amount, 2) }}</td>
                    <td class="text-center">
                        @if($return->refund_status === 'processed')
                            <span class="badge bg-success" style="font-size:11px;">✓ Processed</span>
                        @else
                    ...
                    <td class="text-center">
                        <div class="btn-group btn-group-sm">
                            <a href="{{ route('service-returns.show', $return) }}" class="btn btn-outline-primary" title="View"><i class="fas fa-eye"></i></a>
                            @if($return->status === 'pending')
                            <a href="{{ route('service-returns.edit', $return) }}" class="btn btn-outline-warning" title="Edit"><i class="fas fa-edit"></i></a>
                            @endif
                        </div>
                    </td>
                </tr>
                ...
                <tr>
                    <td colspan="8" class="text-center py-5">
                        ...
                        <p class="text-muted">No service returns found.</p>
                    </td>
                </tr>
                @endforelse
            </tbody>
