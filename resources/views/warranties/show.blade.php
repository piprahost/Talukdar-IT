...
<div class="row g-3 justify-content-center">
    <div class="col-md-10">
        {{-- Status Banner (same style as result.blade.php) --}}
        ...
        {{-- Warranty Timeline Progress Bar --}}
        ...
        <div class="row g-3 mb-3">
                ...
                <div class="table-card h-100">
                    ...
                    <div class="p-3">
                        <table class="table table-sm table-borderless mb-0" style="font-size:14px;">
                            <tr><td class="text-muted ps-0" style="width:40%;">Barcode</td><td class="pe-0"><code style="background:#f3f4f6;padding:2px 6px;border-radius:4px;font-size:12px;">{{ $warranty->barcode ?? 'N/A' }}</code></td></tr>
                            <tr><td class="text-muted ps-0">Product</td><td class="pe-0 fw-semibold">{{ $warranty->product->name }}</td></tr>
                            <tr><td class="text-muted ps-0">SKU</td><td class="pe-0"><code style="font-size:12px;">{{ $warranty->product->sku }}</code></td></tr>
                            <tr><td class="text-muted ps-0">Category</td><td class="pe-0">{{ $warranty->product->category->name ?? 'N/A' }}</td></tr>
                            <tr><td class="text-muted ps-0">Brand</td><td class="pe-0">{{ $warranty->product->brand->name ?? 'N/A' }}</td></tr>
                        </table>
                    </div>
                </div>
            </div>
            <div class="col-md-6">
                <div class="table-card h-100">
                    <div class="table-card-header">
                        <h6><i class="fas fa-user me-2"></i>Customer & Sale</h6>
                    </div>
                    <div class="p-3">
                        <table class="table table-sm table-borderless mb-0" style="font-size:14px;">
                            <tr><td class="text-muted ps-0" style="width:40%;">Customer</td><td class="pe-0">{{ $warranty->customer ? $warranty->customer->name : ($warranty->sale->customer_name ?? 'Walk-in Customer') }}</td></tr>
                            @if($warranty->sale)
                            <tr><td class="text-muted ps-0">Invoice</td><td class="pe-0"><a href="{{ route('sales.show', $warranty->sale) }}" class="text-primary fw-semibold">{{ $warranty->sale->invoice_number }}</a></td></tr>
                            <tr><td class="text-muted ps-0">Sale Date</td><td class="pe-0">{{ $warranty->sale->sale_date->format('d M Y') }}</td></tr>
                            @endif
                        </table>
                    </div>
                </div>
        ...
        <div class="table-card">
            <div class="table-card-header">
                ...
                <div>
                    <a href="{{ route('warranties.verify') }}" class="btn btn-sm btn-primary me-2"><i class="fas fa-search me-2"></i>Verify Another</a>
                    <a href="{{ route('warranties.index') }}" class="btn btn-sm btn-outline-secondary">Back to List</a>
                </div>
            </div>
        </div>
    </div>
